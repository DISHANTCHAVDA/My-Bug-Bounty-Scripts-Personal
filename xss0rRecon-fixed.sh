...
run_step_3() {
    ...
    if [[ "$user_choice" == "Y" ]]; then
        ...
        # Step 1: Passive FUZZ domains with wordlist (dnsbruter with safe fallback)
        show_progress "Passive domain discovery (dnsbruter with automatic fallback)"
        python3 -m venv .venv
        source .venv/bin/activate

        if dnsbruter -d "$domain_name" -w subs-dnsbruter-small.txt -c 150 -wt 80 -rt 500 -wd -ws wild.txt -o output-dnsbruter.txt; then
            echo "[+] dnsbruter completed successfully"
        else
            echo "[!] dnsbruter failed (known Python/asyncio issue)"
            echo "[*] Falling back to subfinder + assetfinder (automatic)"

            subfinder -d "$domain_name" -silent > output-dnsbruter.txt
            assetfinder --subs-only "$domain_name" >> output-dnsbruter.txt
        fi
        sleep 5
